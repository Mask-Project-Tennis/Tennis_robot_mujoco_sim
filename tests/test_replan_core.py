...
def _build_planning_setup(
...
) -> tuple[PlanningEnv, dict, PlanningEnv, AsyncReplanner, np.ndarray, np.ndarray]:
    """构建 do_replan 直接调用所需的 env / cfg / env_plan / replanner。
    ...
    Returns:
        (env, cfg, env_plan, replanner, d_hat, v_hit_desired)。
    """
    env = _build_env()
    ...
    replanner = AsyncReplanner(env, do_replan, cfg, state=ReplanState(), model_path=model_path)
    ...
    return env, cfg, env_plan, replanner, d_hat, v_hit_desired
...
def test_replan_reachable_ball() -> None:
    ...
    env, cfg, env_plan, replanner, d_hat, v_hit_desired = _build_planning_setup(ball_pos, ball_vel)
    ...
    state = ReplanState()
    result = do_replan(request, env_plan, state, cfg)
