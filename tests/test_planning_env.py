...
测试 PlanningEnv 实现 RobotEnv Protocol 的全部接口。
PlanningEnv 是 MPC 规划计算环境（MuJoCo 纯计算），不接触真机硬件。
"""
...
def env() -> PlanningEnv:
    """提供 PlanningEnv 实例。"""
    return PlanningEnv(MODEL_PATH, dt=0.005)


class TestPlanningEnvBasic:
    """PlanningEnv 基础接口测试。"""

    def test_model_loads_right_arm_6_dof(self, env: PlanningEnv):
        """S1: 模型加载成功，右臂 6 DOF。"""
        assert env.NQ == 6
        assert env.NX == 12
        assert env.NU == 6
    ...
    def test_satisfies_robot_env_protocol(self, env: PlanningEnv):
        """S2: isinstance(PlanningEnv, RobotEnv) 通过。"""
        assert isinstance(env, RobotEnv)

    def test_get_arm_state_zero_returns_12(self, env: PlanningEnv):
    ...
    def test_get_ee_pos_reasonable(self, env: PlanningEnv):
    ...
    def test_set_arm_state_changes_ee_pos(self, env: PlanningEnv):
...
class TestPlanningEnvPhysics:
    """PlanningEnv 物理仿真测试。"""

    def test_step_from_state_torque_mode(self, env: PlanningEnv):
    ...
    def test_get_ee_jacp_shape(self, env: PlanningEnv):
        """S7: get_ee_jacp 返回 (3, 6)。"""
        env.reset(np.zeros(6))
        jacp = env.get_ee_jacp()
        assert jacp.shape == (3, 6)

    def test_get_ee_normal_is_unit_vector(self, env: PlanningEnv):
...
class TestPlanningEnvPositionMode:
    """PlanningEnv 位置模式测试。"""

    def test_configure_actuator_mode_position(self, env: PlanningEnv):
    ...
    def test_position_mode_step_with_feedforward(self, env: PlanningEnv):
