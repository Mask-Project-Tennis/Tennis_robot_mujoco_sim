...
RM-65 双臂机器人网球击打项目。使用 **MPC（模型预测控制）** 作为外层闭环框架，**iLQR（迭代线性二次调节器）** 作为内层轨迹优化求解器，**空间走廊式终端代价（代码中称 Tube）** 实现时空鲁棒性，**多层安全滤波**保障关节/TCP 约束。
...
论文的全部统计与图表由三个脚本从已跟踪的逐 episode 结果重建，无需重跑仿真
（下面的单 episode 运行为可选自检）。

### 1. 环境

见 [环境安装](#环境安装)。C++ 加速模块可选（有 NumPy 回退），论文数字使用了它。
...
预期：末行 `__RESULT__` 中 `hit_type=active`（单机约 1.4 s；误差数值跨平台可能略有差异）。

### 3. 复现全部统计

```bash
python scripts/extract/paired_stats.py   # 逐 episode 配对分析 -> 统计 JSON
python scripts/extract/aux_stats.py      # 敏感性 sweep + TCP 超限诊断
```

`paired_stats.py` 写出 `experiment_data/paper_stats_active.json`（论文使用的统计
JSON，跟踪入库的基准副本）。`aux_stats.py` 写出 `experiment_data/aux_stats.json`，
复现敏感性 sweep（93.9–94.9%；corridor-only 84.8%）与 TCP 超限诊断（19.2% 的
episode 峰值超过 1.0 m/s，mean 1.09，p90 1.01）。
...
写出 `repro_outputs/*.pdf`（Fig. 1、Fig. 3–8）与
`repro_outputs/table_data/table1_comparison.tex` + `table2_ablation.tex`（Table I–II），
与投稿版本一致。
...
| E1 速度扫描，7 m/s | 84.3 % |
| E2 TCP 1.0 m/s 限速，7 m/s | 35.9 %（无约束 84.4 %） |
| 敏感性 sweep（9 m/s） | 93.9–94.9 %（corridor-only 84.8 %） |
| TCP 超限比例 | 19.2 % 的 episode（mean 1.09，p90 1.01 m/s） |

### 6. 数据对照表

| 实验 | 目录 | 产出 |
|---|---|---|
| E1 能力扫描 | `experiment_data/exp15_speed_v2` | Fig. 5、正文 |
...
| E6 实时预算 | `experiment_data/exp18_fig_assets/timing.json` | Fig. 8 |
...
鲁棒层不对时间变化的不确定性管道建模，而是改造终端代价以容纳一条可行击球带：

```
候选击球窗口：以预测最优击球步 best_k 为中心，半宽 window_half_ms（默认 50 ms）

空间走廊（垂直偏离 hinge loss，不绑定时间-空间对应）:
  cost = ½·s_k·Q_p_tube·max(0, ‖P_⊥(p_ee - p_ball,k)‖ - r_racket)²
  走廊半宽 = 拍半径（0.12 m），走廊内零代价

Softmin 终端聚合（候选球状态，锐度 β）:
  l_N^sm = softmin_i ( c_i )，所有候选在共享终端状态上求值
```

速度方向与拍面法向由终端代价承担、不在走廊项内——走廊保持纯空间松弛。
代码模块名 `Tube*` 为历史命名，论文中称「空间走廊（spatial corridor）」。

### 多层安全滤波

```
1. 关节约束：位置/速度/加速度/力矩 四重限制
2. TCP 速度硬限制（仿真默认 1.8 m/s）
3. X 平面墙：臂不越过身体中线（X ≥ -0.1）
4. 逐步安全滤波：β 递降 [0.8, 0.6, 0.4, 0.2, 0.0]；全部失败 → 紧急制动
```
...
| `--window-ms` | float | 50.0 | 候选时间窗半宽 (ms) |
| `--softmin-beta` | float | 5.0 | Softmin 温度参数 |
| `--corridor-radius` | float | 0.12 | 空间走廊半宽 (m) |
...
│
├── tests/                                # 单元测试（662 tests）
├── experiment_data/                      # 逐 episode 结果/日志/图表资产（跟踪子集）
├── docs/                                 # 技术文档
└── paper/                                # 论文 LaTeX 工程（本地，不入库）
```
