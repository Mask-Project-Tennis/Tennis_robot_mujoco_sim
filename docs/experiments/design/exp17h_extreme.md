...
> **状态**: ✅ 已完成（数据 2026-09-11 22:41，24,000 runs；报告见
> [reports/2026-09-12_exp17h_extreme.md](../reports/2026-09-12_exp17h_extreme.md)）
...
实际走向 = 混合：**H_A 拒绝、H_B 证实**（详见报告）——

| 假设 | 结果 | 论文写法 |
|------|------|---------|
| H_A（增益随 s 单调） | ❌ 拒绝 | 如实写「走廊在 s≤0.2 显著（+3.57pp），更大偏差下回落至 2-3pp 且不显著」——覆盖上限与走廊半宽 0.12 m 同量级 |
| H_B（紧余量下放大） | ✅ 证实 | 条件性结论升级：「走廊独立价值随时间余量紧张度放大」（9 m/s +3.57pp → 12 m/s +6.91pp，两个显著点） |
